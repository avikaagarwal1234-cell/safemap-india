// Local Database Seeding Script for SafeMap India
// Inserts 50 realistic fake incidents into local storage for testing
...
function seedTestData() {
  try {
    ...
    // Store in a JSON file for demo purposes
    const dataPath = path.join(__dirname, '..', 'src', 'data', 'test-incidents.json');
    ...
    fs.writeFileSync(dataPath, JSON.stringify(testIncidents, null, 2));
    
    console.log(`✅ Successfully stored ${testIncidents.length} incidents in ${dataPath}`);
    
    // Display summary
    const summary = {
      totalIncidents: testIncidents.length,
      cities: [...new Set(testIncidents.map(inc => inc.city))].length,
      incidentTypes: [...new Set(testIncidents.map(inc => inc.type))].length,
      verifiedIncidents: testIncidents.filter(inc => inc.verified).length,
      avgSeverity: (testIncidents.reduce((sum, inc) => sum + inc.severity, 0) / testIncidents.length).toFixed(1)
    };
...
seedTestData()
  .then(() => {
    ...
    console.log('🗺️  Refresh your browser to see the populated incidents on the map.');
    console.log('💡 The data is stored in localStorage and will be available for the session.');
  })
  .catch((error) => {
